...
namespace Velvet.Tests
{
    ...
    /// <item>Ownership tracking mirrors the identity set the node-array pool already keeps: the V.* factories
    /// accept caller-supplied props bags / event arrays that a component may cache and reuse across renders, so
    /// returning an instance the pool did not itself rent out must not clear it or admit it into the shared
    /// pool — only pool-owned instances may be cleared and recycled. A release scope stages returns so a bag
    /// retired mid-pass cannot be rented back until the scope ends, preventing a second retirement of the same
    /// bag within one pass from recycling a live renter's object.</item>
    /// </list>
    /// </summary>
    [TestFixture]
