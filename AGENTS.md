...
This file is the shared repository guidance for Claude Code, Codex, and Cursor.

Harness sources and generated client files are documented in [CONTRIBUTING.md](CONTRIBUTING.md#agent-harness). Edit the owning source; shared content is linked or referenced. Run `python3 scripts/harness/sync.py` only after changing hook registration or agent discovery metadata.
