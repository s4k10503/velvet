...
- **Run a subset / single fixture:** add `-testFilter "Velvet.Tests.SomeFixture"` (semicolon-separates multiple; matches fully-qualified class or method names). A **file's** name is neither: a test file here may declare several fixture classes, and some declare none matching their own stem, so a filter derived from the names of the files a change touched runs a set nobody chose and reports green over it. `.claude/hooks/refuse/filter_naming_no_fixture.py` refuses a filter naming a class nothing declares, and names the classes a filter leaves out of the files it does name.
