...
- **Run a subset / single fixture:** add `-testFilter "Velvet.Tests.SomeFixture"`. The value is a regex over each case's full name rather than a name looked up, and semicolons separate several without trimming, so a filter can select a set other than the one it reads as and the run still reports green — `.claude/skills/unity-tests/SKILL.md` owns what each spelling costs.
