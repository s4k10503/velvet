...
- **Run a subset / single fixture:** add `-testFilter "Velvet.Tests.SomeFixture"` (semicolon-separates multiple, and the runner trims nothing, so a space after a semicolon belongs to the value behind it). Each value is an **unanchored regular expression matched against a test's full name** — its namespace, fixture and case joined by dots, with a plus rather than a dot before a nested fixture — and against each suite above it: the namespace, the fixture, and the assembly under its compiled file name. A value matching one of those selects everything under it, and one starting with `!` excludes rather than selects. A type that only nests fixtures is reported as a suite carrying no case of its own: its nested fixtures are reported beside it under the namespace rather than below it, so a value equal to its own name selects nothing while the unanchored spelling still reaches them. A **file's** name is none of those names either: a test file here may declare several fixture classes, and some declare none matching their own stem, so a filter derived from the names of the files a change touched selects a set nobody chose and reports green over it. `.claude/hooks/refuse/filter_selecting_no_test.py` refuses a value it reads and finds selects nothing — it declines several shapes rather than answering for every value, and its own header is where they are listed — and names the fixtures a selecting one leaves out of the files it reaches.
