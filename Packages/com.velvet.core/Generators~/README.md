...
Pinning the runtime instead was rejected. It needs `DOTNET_ROLL_FORWARD` set in the environment of every build, which no project file can express, so a contributor's IDE build would silently produce different bytes — a guard whose correctness depends on an environment variable nobody sets is the failure mode this exists to remove.
