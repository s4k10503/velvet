...
- **killed (the suite did not finish)** — the editor was killed at `--timeout`, where the baseline had finished in a third of that or less, so the run reads the mutation rather than the bound as what kept it running. A suite that never finishes is not one that still passes, so this counts as answered, and its detail names the bound and the baseline's wall clock rather than a failing test.
