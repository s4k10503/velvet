...
Widening the operator set is the obvious answer and is not free: a statement-deletion operator over the same twenty-four commits would add 113 mutants to the 211 — around half as much wall clock again — to reach 113 of the 340 unreached lines. It also has to emit compilable C#, which deleting a declaration the rest of the method reads does not.
