...
namespace Velvet.SourceGenerators.Tests
{
    ...
    public sealed class MemoOverloadGeneratorTests
    {
        ...
        // Regenerated per test rather than cached in a static field. A static initialiser runs once per process,
        // so a mutation-testing pass attributes the whole generator to whichever test loaded the class first and
        // then runs that one test per mutant — most of the file reports as unkillable for a reason that has
        // nothing to do with the assertions.
        private static string Generated => MemoOverloadGenerator.GenerateOverloads();

        [Fact]
        public void Given_TheGeneratedSource_When_ItIsEmitted_Then_TheFileHeaderOpensTheVelvetPartial()
        {
            // Arrange
        ...
        private static void AssertMethodMatchesSnapshot(string methodName, int arity)
        {
            ...
            var actual = ExtractMethodBlock(Generated, signature);
