...
{
    /// <summary>
    /// A project the solution does not name, and that no member reaches by <c>ProjectReference</c>, is built
    /// by nothing: its analyzers never run, while every guard that reads its sources off disk keeps reporting
    /// on them. The suite stays green for the same reason it would if the project were deleted, which is the
    /// failure mode the opt-in guards were added to close — one level further out, where they cannot see it.
    /// <para>
    /// The <c>ProjectReference</c> half is measured, not assumed: a project omitted from the solution but
    /// referenced by a member does compile, and a syntax error in it fails the build. Membership is a
    /// convention this repository holds to anyway, since a non-member opens in no IDE and restores on nobody's
    /// machine, but it is reachability that decides whether a compiler ever sees the file.
    /// </para>
    /// </summary>
    /// <remarks>
    ...
    public sealed class SolutionProjectMembershipDriftTests
    {
        ...
        {
            // What the membership comparison structurally cannot see is the disk enumeration narrowing: a
            // smaller set of projects is still a subset of what the solution names, so it reports an empty
            // difference and the count canary above stays non-zero. Excluding one real project from the
            // search leaves this the only case red.
            // Arrange
            var solution = File.ReadAllText(SolutionPath());
        ...
        /// <summary>
        /// The project files the repository carries under the solution's own directory. <c>bin</c> and
