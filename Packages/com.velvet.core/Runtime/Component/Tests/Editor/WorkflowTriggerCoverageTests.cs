...
{
    /// <summary>
    /// Checks that the workflow running this suite starts at all for the files the suite reads. A workflow
    /// that never starts leaves a pull request showing the same absence of a red check as one that ran and
    /// passed, so a guard outside the path set keeps reporting nothing until some later, unrelated change
    /// happens to carry a file inside it — and then names that change as the culprit.
    /// </summary>
    [TestFixture]
    internal sealed class WorkflowTriggerCoverageTests
    {
        // One workflow, because a fixture can only speak for the job that runs it. Which job runs the drift
        // guards in the Generators~ solution is not derivable from anything this assembly reads, and writing
        // that mapping out by hand would restate exactly the claim this fixture exists to derive.
        ...
        // Every repo file the workflow names. The one that matters is scripts/assert-no-inconclusive.py:
        // nothing else in the repository runs it, and its failure mode is passing a run it should have
        // failed, which is invisible until a test starts skipping.
