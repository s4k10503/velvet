...
namespace Velvet.Tests
{
    ...
    /// </summary>
    /// <remarks>
    /// Also covers the convergence contract for a staggered AnimatePresence list re-keyed across tabs: after a
    /// churn and a real settle on a known tab, every one of that tab's cards must be present and every prior-tab
    /// ghost must be gone — no card disappears, none pile up. This regressed on the exit-completion re-render
    /// bailing on an auto-memoized boundary: exit completion re-renders the boundary via ScheduleRerender, but
    /// the boundary's own hook inputs are unchanged, so an auto-memoized boundary returns its cached VNode and
    /// the reconciler bails — the AnimatePresence never re-expands and the finished ghost lingers forever
    /// (masked while tabs keep toggling; exposed once interaction stops). The fix invalidates the boundary's
    /// memo cache before that re-render (InvalidateMemoCache); reverting it strands the prior-tab ghosts so the
    /// settled set never reduces to the expected cards.
    /// </remarks>
    [TestFixture]
    internal sealed class AnimatePresenceAnimationTests : SimulatedPanelTestBase
    {
        ...
        #region List Convergence (staggered re-key across tabs)
