...
namespace Velvet.Tests
{
    ...
    /// <item>The forwarded ref is part of a memoized forwardRef component's identity, not its props: when the
    /// parent passes a DIFFERENT <see cref="Ref{T}"/> instance with equal props, the child must still re-run
    /// <see cref="Hooks.UseImperativeHandle"/> so the new ref receives the handle — otherwise the props-bail
    /// would swallow the ref change and the new ref would never be populated.</item>
    /// </list>
    /// </summary>
    /// <remarks>
    ...
    internal sealed class ComponentMemoPropsBailTests
    {
        ...
        #region Memoized forwardRef identity
