...
namespace Velvet
{
    ...
    internal static class SilhouetteFace
    {
        ...
        // Builds the element's rounded-rect path (inset on all sides), shearing every point — a shear is
        // affine, so transforming the bezier control points transforms the curve exactly. tanX = tanY = 0
        // yields an upright rounded rect. Shared by both silhouette layers' fill + border draws.
            ...
            float w, float h, float tanX, float tanY)
        {
            ...
            var tl = Mathf.Clamp(rs.borderTopLeftRadius - inset, 0f, maxR);
            var tr = Mathf.Clamp(rs.borderTopRightRadius - inset, 0f, maxR);
            var br = Mathf.Clamp(rs.borderBottomRightRadius - inset, 0f, maxR);
            var bl = Mathf.Clamp(rs.borderBottomLeftRadius - inset, 0f, maxR);
            ...
            if (tr > 0f)
            {
                p.BezierCurveTo(S(x1 - tr + (Kappa * tr), y0), S(x1, y0 + tr - (Kappa * tr)), S(x1, y0 + tr));
            }
            p.LineTo(S(x1, y1 - br));
            if (br > 0f)
            {
                p.BezierCurveTo(S(x1, y1 - br + (Kappa * br)), S(x1 - br + (Kappa * br), y1), S(x1 - br, y1));
            }
            p.LineTo(S(x0 + bl, y1));
            if (bl > 0f)
            {
                p.BezierCurveTo(S(x0 + bl - (Kappa * bl), y1), S(x0, y1 - bl + (Kappa * bl)), S(x0, y1 - bl));
            }
            p.LineTo(S(x0, y0 + tl));
            if (tl > 0f)
            {
                p.BezierCurveTo(S(x0, y0 + tl - (Kappa * tl)), S(x0 + tl - (Kappa * tl), y0), S(x0 + tl, y0));
            }
            p.ClosePath();
        }
    }
    ...
    // its lifecycle (TryStash on attach, SyncOnPatch on patch, Release on detach) is driven by that binding.
    internal sealed class SilhouetteFaceStash
    {
        ...
        // caller's CustomStyleResolvedEvent / GeometryChangedEvent retries after attach.
        public void TryStash(VisualElement element)
        {
            var inlineBg = element.style.backgroundColor.value;
            if (SilhouetteFace.IsUnset(inlineBg) && element.panel == null)
            {
                return;
            }
            Capture(element);
        ...
        // - the inline BACKGROUND OR BORDER slot holds a fresh (non-sentinel) value → the resolver overwrote us
        //   (e.g. border-[#fff] → border-[#f00] while bg stays suppressed): re-capture both and re-suppress;
        // - both slots still hold our sentinel, the class list changed, AND the stash is USS-driven → a USS-side
        //   color may have changed beneath the sentinel: release it and let the next style resolution re-stash;
        ...
        public void SyncOnPatch(VisualElement element, bool classesChanged)
        {
            ...
            var inlineBg = element.style.backgroundColor.value;
            var inlineBorder = element.style.borderLeftColor.value;
            if (!SilhouetteFace.IsSentinel(inlineBg) || !SilhouetteFace.IsSentinel(inlineBorder))
            {
                Capture(element);
                ApplySuppression(element);
                return;
            ...
            // Release-and-re-stash ONLY when NEITHER face is inline-driven: releasing nulls the shared inline
            // slots, so for an inline-driven face that would drop the authored color (the double-image regression).
            // Documented limitation of the shared-slot scheme: a MIXED element (one inline face + one USS face)
            // whose USS face color changes beneath the sentinel is not picked up until the next resolver write —
            // a full per-face release would need separable suppression.
