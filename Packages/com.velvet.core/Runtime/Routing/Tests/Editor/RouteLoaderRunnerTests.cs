...
namespace Velvet.Tests
{
    ...
    /// <item>A Suspend loader of a promoted round returns immediately with <c>allCompleted</c> false and runs
    /// in the background, firing <c>OnSuspendLoaderCompleted</c> on success or <c>OnSuspendLoaderFailed</c> on
    /// failure; a failure is also recorded per-path in the round's errors, symmetrically with the Await
    /// path.</item>
    ...
    /// <item>The loader receives a cancelable token that retiring its round cancels, and a loader that
    /// answers that token by succeeding rather than throwing is still treated as a torn-down round.
    /// Disposal retires the promoted round and the newer one alike.</item>
    ...
    internal sealed class RouteLoaderRunnerTests
    {
        ...
        public void Given_APromotedRoundAndANewerOne_When_TheRunnerIsDisposed_Then_BothTokensAreCancelled()
        {
            ...
            runner.Promote(runner.RunLoadersSync(
                ...
                {
                    live = ct;
                    return UniTask.FromResult<object>("live-data");
                }),
            ...
            runner.RunLoadersSync(
                ...
                {
                    newer = ct;
                    return UniTask.FromResult<object>("newer-data");
                }),
                CancellationToken.None);
