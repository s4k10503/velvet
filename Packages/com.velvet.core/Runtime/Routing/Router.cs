...
namespace Velvet
{
    ...
    public sealed class Router : IDisposable
    {
        ...
        /// navigation resolved. At most four redirects are followed: a fifth is refused and the navigation
        /// ends with <see cref="NavigationResult.Error"/>.
        /// </summary>
            ...
            CancellationTokenSource? takeover)
        {
            ...
            else
            {
                ...
                if (takeover != null)
                {
                    // Cancel whoever held the claim so it unwinds (Blocker.CheckAsync await observes the
                    // cancellation). Dispose of the prior CTS is left to the prior navigation's own finally —
                    // disposing here would double-dispose and confuse ownership, and the synchronous Cancel
                    // chain may already run the prior finally before we proceed.
