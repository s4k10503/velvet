...
namespace Velvet.Tests
{
    ...
    /// down mid-exit (a pooled element must not keep receiving tick writes); complete an exit whose
    /// variants animate no channel of that driver exactly once instead of replaying forever against
    /// the boundary's per-pass exit bookkeeping; and never hand off to a reversal on a teardown
    /// cancel. The bezier suite additionally pins the mid-tick cancel's reversal hand-off (Retarget +
    /// move into the enter map) on an actively-ticking exit.
    /// </summary>
    [TestFixture]
