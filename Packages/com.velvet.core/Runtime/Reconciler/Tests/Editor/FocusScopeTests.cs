...
namespace Velvet.Tests
{
    ...
    /// panel so a focus controller exists — unlike <see cref="FocusScopeTests"/> above, this needs the genuine
    /// windowed panel (guarded by <see cref="TestGraphics.IgnoreIfHeadless"/>) rather than the headless
    /// <see cref="HeadlessEditorPanelHost"/> panel, because the regression is specifically about a real Unity-engine
    /// blur/detach dispatch racing the reconciler's removal, which the synthetic panel does not reproduce. GWT, one
    /// assert per case.
    /// </summary>
    [TestFixture]
