...
token is released once the run that launched the loaders has returned and every `Suspend` loader it
started has finished, so a loader unwinding on the cancellation reads its token as cancelled.
