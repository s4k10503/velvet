...
| [styling-flexbox-and-gap.md](styling-flexbox-and-gap.md) | Flexbox direction (`flex` defaults to column, not row), which direction overrides a variant can and cannot apply (`flex-col md:flex-row` works, the mirror does not), and `gap-*` gotchas vs Tailwind — the leading-margin polyfill, its edge flip on `flex-row-reverse` / `flex-col-reverse` (and `space-x-reverse` / `space-y-reverse`), and the wrap half-margin hybrid |
