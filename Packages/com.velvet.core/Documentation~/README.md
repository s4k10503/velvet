...
| [styling-variants.md](styling-variants.md) | The variant set (state / theme `dark:` / responsive `sm:`…`2xl:` / relational `group-`·`peer-` / stacked), the precedence order a class or arbitrary-value payload is ranked by against the base utility, the `!` important modifier, and container queries (`@container`, the `container-type: inline-size` equivalent) |
