...
| `useTransition()` | `Hooks.UseTransition()` | React parity on the returned tuple: `(isPending, startTransition)` in the same order as React's `[isPending, startTransition]`. The callback marks the updates it schedules synchronously, whichever component owns the state they write — a setter received as a prop included — matching React's ambient transition flag. Also as in React, the marking ends where the callback returns: an `async` action keeps `isPending` true until the task completes, but its post-`await` updates fall outside the scope its callback opened, so they take the Normal lane unless they land in some other scope — a discrete handler's, or a further `startTransition` call, which is what React's reference tells callers to write. An update from elsewhere that lands while the action awaits keeps its own priority, discrete input included, and a second `UseTransition()` slot started there is an independent transition with its own `isPending`. `isPending` follows the slot's own updates, not the Transition lane: a callback that scheduled no update settles as soon as it returns, and a `UseDeferredValue` in the same component holding that lane does not keep the flag lit. It also belongs to the component that declares the hook and follows that component's own lanes, so a transition whose updates all go to other components' state settles as soon as its callback returns |
