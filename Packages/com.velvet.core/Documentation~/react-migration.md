...
| `useTransition()` | `Hooks.UseTransition()` | React parity on the returned tuple: `(isPending, startTransition)` in the same order as React's `[isPending, startTransition]`. The callback marks the updates it schedules synchronously, whichever component owns the state they write — a setter received as a prop included — matching React's ambient transition flag. As in React, the marking ends where the callback returns: an `async` action keeps `isPending` true until the task completes, but its post-`await` updates fall outside the scope its callback opened, so they take the Normal lane unless they land in some other scope — a discrete handler's, or a further `startTransition` call, which is what React's reference tells callers to write for them. An update from elsewhere that lands while the action awaits keeps its own priority, discrete input included, and a second `UseTransition()` slot started there is an independent transition with its own `isPending`. `isPending` follows the updates the slot's own callback scheduled, not the Transition lane: a callback that scheduled no update settles as soon as it returns, and a `UseDeferredValue` in the same component holding that lane does not keep the flag lit. Where those updates landed on other components, the flag stays lit until each of them commits; for an `async` action it stays lit until the task completes as well, and the declaring component re-renders once whichever of the two lands last. **Velvet deviation:** nothing renders purely because `isPending` turned true, where React re-renders the component on that alone. Whichever render the same interaction already causes is what observes the flag here — the urgent update a click also makes, or an ancestor's — so a component whose only update is a synchronous transition's own shows no pending state at all: the render that commits the transition is the first one it gets, and the flag is already false there. An `async` action does show it, from the commit of the work its callback queued until its task completes |
