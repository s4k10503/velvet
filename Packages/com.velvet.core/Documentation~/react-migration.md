...
>
> A component keeps the elements it rendered wherever its siblings move it, as in React: a keyed component that a reorder or an inserted sibling moves takes its elements to its new slot, whether it is written into an element, under a `V.Fragment` or at the root, and what it rendered into them or into its `V.Portal` keeps its state. An unkeyed element is matched by its position in the output of the component that renders it, counted from where that output begins, so what a sibling component renders does not shift it, while a change in how many elements a component written earlier in that output renders does, as a `null` does below.
>
