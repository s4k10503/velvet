...
| `useTransition()` | `Hooks.UseTransition()` | React parity on the returned tuple: `(isPending, startTransition)` in the same order as React's `[isPending, startTransition]`. An `async` action keeps `isPending` true and its own post-`await` updates on the Transition lane until the task completes; during that window an update from a discrete input on the same component keeps its urgent priority unless the handler wraps it in a `startTransition` of its own, and a second `UseTransition()` slot started there is an independent transition with its own `isPending` |
