...
- **Run it detached.** Killing the editor does not kill the player. The orphan then holds the profiler port and the *next* player run builds, launches, connects and dies with "No activity received from the player in 600 seconds", which reads as contention from somewhere else entirely.
