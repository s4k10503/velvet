...
- **Reap with `ps -Ao command= | grep -c '^/.*PlayerWithTests'`, anchored.** Two separate traps here. A pattern aimed at the editor misses the orphan entirely, whose command line is `<worktree>/Temp/UnityTempFile-*/PlayerWithTests/PlayerWithTests.app` and carries neither `Unity` nor `-projectPath`. And an unanchored count **counts itself**: `[P]layer…` stops grep matching its own process, but not the `/bin/zsh -c` line carrying the command, so any compound command that also spells the name unbracketed inflates the answer. Measured on an idle machine, the unanchored form reported 2 and the anchored one 0. Anchoring at `^/` is what a shell line cannot satisfy. Two agents drew wrong conclusions from that counter, including that a filtered player run was the problem.
