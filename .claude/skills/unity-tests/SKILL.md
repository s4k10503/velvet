...
- **Do not pass `-testFilter` to a player run.** Three measurements: two filtered runs died at `Test execution timed out. No activity received from the player in 600 seconds`, one of them on a host verified at zero other editors and zero players with a sampler running throughout; one unfiltered run reported 119 results in about three minutes. The filter is not silently dropped — the editor log carries `groupNames` and the fixture is in the build contents — the player simply builds, launches, connects the profiler and then sends no test event. No mechanism is stated here because none was measured. Run unfiltered and read the results you want out of the XML.
