...
**Three harnesses are the exception.** A mutation campaign, a neuter sweep and `base_red_check.py`'s C# lane each wait for the machine to go quiet themselves — a campaign before every mutant, a sweep before every cut, the lane once before it starts — up to `--busy-timeout`, 1800 s by default, and abort when the wait expires. A campaign that aborts mid-loop writes no receipt at all, so what a neighbour costs is every mutant already measured rather than the one it was on. Do not start a suite run while one of the three is in flight.
