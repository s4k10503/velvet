...
The three harnesses the exception below covers — a mutation campaign, a neuter sweep and `base_red_check.py`'s C# lane — hold no editor of their own between their runs, over a campaign's waits, mutations and restores and the lane's whole worktree build, so that count does not see one that is mid-flight. This one does:
...
- Before reporting "no allocations", confirm the instrument works: `GC.GetAllocatedBytesForCurrentThread()` and `GC.GetTotalMemory` both report 0 under Unity's Mono regardless of what happens. Only the `GC.Alloc` recorder measures it — `Unity.PerformanceTesting`'s `.GC()` and `TestUtilities/GCAllocationProbe` both drive it.
- **Do not use `Is.Not.AllocatingGCMemory()`.** It restores the recorder to every thread before reading its counter, so it charges the delegate for allocations made by every other thread in the process. An empty delegate can therefore fail it — measured at 1 window in 20000 on an idle machine, and 2 in 500 with one background thread allocating, so a contributor who tries it once will see it pass and conclude it is fine. What it cannot do is fail for a reason the delegate owns. `GCAllocationProbe.SampleBlocksDuring` reads while the filter still holds. A zero-allocation guard asserts an absence either way, so pair it with a canary that allocates a known amount and asserts a non-zero count — otherwise an instrument stuck at zero satisfies every guard and nothing notices.
