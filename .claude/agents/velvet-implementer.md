...
Then four questions the review will ask. Answering them yourself is cheaper than a round, and each has caught a defect a fully green suite did not.
...
- **If you wrote a case by copying a neighbouring one, name where yours diverges.** A copied block carries the cases that mattered to the family it came from, so the case where your family differs is the one with no counterpart to copy — and no case that exists can redden on it. `basis-[50%]` resolves as a percent length while `grow-[50%]` has to be declined, a ratio having no unit; that percent case is the one the copy left behind, and it made the class a factor of fifty. Then ask what each borrowed term excludes **in your type**, since the same term does not do the same work at another: `PropsDiffTests`' identity term rules out a remount wherever it appears, but at a pooled primitive the pool's reset writes back the very value the assertion reads, so dropping it hands a discard-and-recreate implementation a pass instead of merely weakening the case.
