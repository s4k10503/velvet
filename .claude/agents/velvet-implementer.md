...
- **Three harnesses serialise rather than share the machine**: a mutation campaign, a neuter sweep, and `base_red_check.py`'s C# lane. Each waits for Unity to go quiet — up to `--busy-timeout`, 1800 s by default — and then aborts rather than running beside it.
