...
- **Three harnesses serialise rather than share the machine**: a mutation campaign, a neuter sweep, and `base_red_check.py`'s C# lane. Each waits for Unity to go quiet — up to `--busy-timeout`, 1800 s by default — and then aborts rather than running beside it. `mutation_check.py` and `neuter_check.py` each say so once when the wait starts; `base_red_check.py` prints nothing at all while it waits, so a long silence under that one is the wait rather than a hang.
- **Budget a campaign by the mutant, and do not read the budget off how red a mutant is.** By default it runs the whole platform suite per mutant, and how many tests a mutant reddens does not set its wall clock. Three readings from campaigns run on this repository: one site's `void call removed` failing 135–137 tests took 456–524 s against 94–114 s baselines, while elsewhere mutants failing 159–187 tests ran 93–99 s against a 130 s baseline — faster than green — and elsewhere again killed and surviving mutants alike sat at 84–88 s against an 89 s baseline. The campaign carrying the 503 s mutant also carries a killed one failing two tests in 122 s. So budget from the slowest run you have actually seen rather than from a rate: `mutation_check.py` kills a mutant's editor at `--timeout`, 900 s of wall clock by default, and records that mutant "not measured (timed out)" rather than killed or survived, so a `--timeout` shortened on the strength of a fast campaign loses the slow mutant's answer outright.
