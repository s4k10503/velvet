...
- **Take those runs rather than waiting for a quiet machine.** Measured on one tree: the same EditMode suite, run five times with zero, two and three other Unity runs alive, reported 3943 passed / 0 failed / 0 inconclusive / 0 skipped every time. Duration ranged 114–141 s and did **not** track concurrency — the slowest reading was the least contended one. That is one tree and one suite, so a case whose outcome turns on elapsed time could exist and go unexercised by it: if a case fails and you suspect timing, re-run that case alone before reporting it.
- **A mutation campaign is different, and the reason is the timeout rather than a neighbour.** `mutation_check.py` kills a mutant's editor at `--timeout` — 900 s of wall clock, not test time — and records the mutant **killed**, detailed as "the mutation left something not terminating". A mutant that reddens many tests is slow for that alone: one that broke about 136 tests took 472 s of test time and 503 s of wall clock, reproduced within 60 s across three campaigns hours apart on a quiet machine. So a false kill arrives from a slow mutant rather than from company, and what answers it is raising `--timeout` and distrusting a killed verdict carrying that detail. `wait_for_quiet` refuses loudly before a campaign starts and is silent per mutant, so a campaign never begins beside another editor but can continue beside one.
