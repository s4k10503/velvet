...
- **For each condition you arranged, which term of the assertion depends on it?** The question above audits the assertion; this one audits the arrangement, and they fail apart. A fixture can name a cut that reddens it honestly while a condition it went to the trouble of setting up is decoration — a summary promising a failure reaches the nearest error boundary, in a fixture that registers no boundary and expects the log line printed when there is none; a name saying the task completes off the main thread, with no assertion reading which thread completed it; a hand-rolled state machine calling a builder method the compiler never emits, repairing the defect it exists to catch. Each passed its own cut question. The way to answer this one is to invert the condition and run: register the boundary, complete on the main thread, delete the line. If the assertion does not move, the condition was decoration and the name was a claim nothing holds.
