...
When you do need a suite run, **take it rather than waiting for a quiet machine — but not while a mutation campaign, a neuter sweep or `base_red_check.py`'s C# lane is in flight**, which a neighbouring run can end rather than delay. The `unity-tests` skill carries both readings: one loaded run against one quiet one, where the load cost wall clock and moved no count, and what a neighbour costs those three. Read that skill before running anything; it carries the traps that otherwise produce confident wrong answers.
