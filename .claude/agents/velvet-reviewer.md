...
When you do need a suite run, **take it rather than waiting for a quiet machine.** Measured on one tree: the same EditMode suite, run five times with zero, two and three other Unity runs alive, reported 3943 passed / 0 failed / 0 inconclusive / 0 skipped every time. Duration ranged 114–141 s and did **not** track concurrency — the slowest reading was the least contended one. So neighbours did not change what the suite reported, and did not reliably cost wall clock either. That is one tree and one suite; a case whose outcome turns on elapsed time could exist and go unexercised by it, so if a case fails and you suspect timing, re-run that case alone before reporting it.

Use your own worktree and your own `-testResults` path, never a shared one, and read the `unity-tests` skill first — it carries the traps that otherwise produce confident wrong answers.

**Do not run `mutation_check.py` or `neuter_check.py`.** Both write to tracked sources and undo it in a `finally`, which does not run when a command is killed at the Bash tool's cap — and you arrive with your working directory on the checkout other agents share, so a killed run leaves a mutation or a cut in their tree. Ask the coordinator for a campaign's result rather than taking one.
