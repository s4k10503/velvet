...
- **CI takes the Unity evidence for a pull request**: `Test ▸ unity-tests` runs both suites, `Test ▸ base-red` runs the cases the branch wrote on the merge base, and the mutation campaign runs over the change — `CONTRIBUTING.md` owns what each one fails on. A local Unity run is optional: take a **targeted** `-testFilter` run when you want a failure text before the branch is pushed, and where you took none, say so and name the cases the CI run has to show red on the base. Run the cheap guards either way. **Do not run the full EditMode and PlayMode suites** — CI runs both in parallel on a checkout nobody warmed, with `assert_results_from_this_tree.py` and `assert_no_inconclusive.py` after each, in less wall clock than one local pass costs. A local full run seeds its `Library` from another checkout, which is the failure those two asserters exist to catch. Read the `unity-tests` skill for how to run anything at all — it carries the traps that otherwise produce confident wrong answers.
...
Say what you changed, the RED evidence with its failure text — or, where you took no local run, the cases CI has to show red — the suite counts you measured, your documentation and CHANGELOG judgements, and — separately — **anything you found but did not fix**. That last section is often the most valuable thing in the report; do not omit a defect because it was out of scope.
