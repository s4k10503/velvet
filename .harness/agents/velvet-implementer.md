...
- **A timed-out mutant is not a survivor.** `mutation_check.py` kills a mutant's editor at `--timeout`, 900 s by default, and the mutation section of `Packages/com.velvet.core/Generators~/README.md` owns which of two verdicts that records. One of them, `not measured (timed out)`, is neither killed nor survived: raise `--timeout` and take that mutant again rather than reporting it as a hole the tests left open.
