...
- **Prove each new test RED without your fix and GREEN with it.** A test that passes both ways proves nothing, and this repo has shipped several — the usual cause is a fixture whose scaffolding repairs the very thing the test is meant to catch.
...
- **CI takes most of that evidence, and not all of it**: `Test ▸ unity-tests` runs both suites, `Test ▸ base-red` runs the cases the branch wrote on the merge base, and the mutation campaign runs over the change. Two kinds of case never show their failure there — one the base cannot build, because it calls a surface your change adds, and one declared `construction` as the bullet above gives — so theirs is yours to show: a **targeted** `-testFilter` run against a cut or the declared perturbation, with the failure text quoted. `CONTRIBUTING.md`'s base-red section owns which cases those are. For the rest a local run is optional, and your report names the cases the CI run has to show red. Run the cheap guards either way. **Do not run the full EditMode and PlayMode suites** — CI runs both in parallel on a checkout nobody warmed, with `assert_results_from_this_tree.py` and `assert_no_inconclusive.py` after each, in less wall clock than one local pass costs. A local full run seeds its `Library` from another checkout, which is the failure those two asserters exist to catch. Read the `unity-tests` skill for how to run anything at all — it carries the traps that otherwise produce confident wrong answers.
- **If you take a local run, take it rather than waiting for a quiet machine — but not while a mutation campaign, a neuter sweep or `base_red_check.py`'s C# lane is in flight**: those three wait for a quiet machine themselves, and a run starting after one's wait has passed is charged to whatever that harness was measuring. The `unity-tests` skill carries both readings — one loaded run against one quiet one, where the load cost wall clock and moved no count, and what a neighbour costs those three — and the count that answers whether one is running, which the editor count does not.
- **A timed-out mutant is not a survivor or a measured kill.** `mutation_check.py` bounds baseline and mutant editors with `--timeout`, 900 s by default. `Packages/com.velvet.core/Generators~/README.md` owns the two timeout verdicts. Both fail the campaign and must be measured again; inspect the log before choosing a larger bound.
...
Then four questions the review will ask. Answering them yourself is cheaper than a round.
...
- **If you wrote a case by copying a neighbouring one, name where yours diverges.** A copied block carries the cases that mattered to the family it came from. Where your family differs, the neighbour has no case, or has one whose verdict is the opposite of yours, and neither is written by copying. `basis-[50%]` resolves as a percent length while `grow-[50%]` has to be declined, a ratio having no unit; the block copied from `basis-` held no case saying so, and a first draft that accepted the percent as a factor of fifty reached review with the suite green. Then ask what each borrowed term excludes **in your type**, since a term that discriminates in the neighbour's type can discriminate nothing in yours: `PropsDiffTests`' identity term separates a restore from a remount only where a remount hands the tree a new instance, and a pooled primitive comes back from the pool as the same instance with the reset value written, so at a `Label` a discard-and-recreate passes the identity and the value both.
...
Say what you changed, the RED evidence — the failure text for each case the base cannot build, and the cases CI has to show red for the rest — the suite counts you measured, your documentation and CHANGELOG judgements, and — separately — **anything you found but did not fix**. That last section is often the most valuable thing in the report; do not omit a defect because it was out of scope.
