...
ps -Ao command= | grep -E '^[^ ]*[Pp]ython[^ ]* .*[ /](mutation_check|neuter_check|base_red_check)\.py' \
  | grep -vE -- '--lane python|--plan|--list|--refusals|--emit-lines|--collect|--report' \
  | grep -c .
```
